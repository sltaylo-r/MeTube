          ...
          <tr>
              ...
              <?php
                  ...
                  if (isset($_POST['contactID'])) {
                    $conID = $_POST['contactID'];
                    $stmt = "UPDATE Contacts SET Status_App = 'Approved' WHERE Contact_ID = '$conID'";
                    mysqli_query($mysqli, $stmt);
                  }
                  $currentUser = $_SESSION['userid'];
                  $request_array = array();
                  $stmt = $mysqli->prepare("SELECT User_ID_One, Contact_ID FROM Contacts WHERE User_ID_Two = '$currentUser' AND Status_App = 'Pending'");
                  if(!$stmt->execute()) { print("Error user information."); } else {
                    $stmt->bind_result($user_id, $contact_id);
                    while($stmt->fetch()) {
                        array_push($request_array, array($user_id, $contact_id));
                    }
                    $stmt->close();
                  }
                  ...
                  echo '<div class="guyFormatted">';
                  for($i = 0; $i < $length; $i++){
                    $tempConID = $request_array[$i][1];
                    echo '<div class="guyFormatted">
                    <form method="post" action="">
                      <input type="hidden" name="contactID" value="'.$tempConID.'">
                      <button type="submit" id="friendButton" name="friendButton">Accept</button>
                    </form>';
                  }
