          ...
          <div class="right-column">
            ...
            <div class="wrapper">
                ...
                <?php
                  ...
                  $video_array = array();
                  $stmt = $mysqli->prepare("SELECT Video_Name, Video_Thumbnail_File, Video_ID FROM Video");
                  if(!$stmt->execute()) { print("Error video information."); } else {
                    $stmt->bind_result($video_title, $video_thumbnail, $video_id);
                    while($stmt->fetch()) {
                        array_push($video_array, array($video_title, $video_thumbnail, $video_id));
                    }
                    $stmt->close();
                  }
